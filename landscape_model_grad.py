...
def parse_args():
    ...
    parser.add_argument("--uplift-rate", type=float, default=0.001, help="Mean tectonic uplift rate (m/yr)")
    parser.add_argument("--uplift-grad-mag", type=float, default=0.0, help="Magnitude of spatial uplift gradient (m/yr per meter)")
    parser.add_argument("--uplift-grad-dir", type=float, default=0.0, help="Direction of uplift gradient (degrees from North/Y-axis)")
    ...
    parser.add_argument("--precip-rate", type=float, default=1.0, help="Mean effective continuous precipitation rate (m/yr)")
    parser.add_argument("--precip-grad-mag", type=float, default=0.0, help="Magnitude of spatial precipitation gradient (m/yr per meter)")
    parser.add_argument("--precip-grad-dir", type=float, default=0.0, help="Direction of precipitation gradient (degrees from North/Y-axis)")
    parser.add_argument("--output-plot", type=str, default="landscape_evolution_grad.png", help="Output plot filename")
...
def main():
    ...
    # Spatial Gradients Setup
    x_c = grid.x_of_node - np.mean(grid.x_of_node)
    y_c = grid.y_of_node - np.mean(grid.y_of_node)

    # Calculate Uplift Field
    theta_u = np.radians(args.uplift_grad_dir)
    uplift_field = args.uplift_rate + args.uplift_grad_mag * (x_c * np.sin(theta_u) + y_c * np.cos(theta_u))
    uplift_field = np.maximum(uplift_field, 0.0) # Prevent subsidence

    # Calculate Precipitation Field
    theta_p = np.radians(args.precip_grad_dir)
    precip_field = args.precip_rate + args.precip_grad_mag * (x_c * np.sin(theta_p) + y_c * np.cos(theta_p))
    precip_field = np.maximum(precip_field, 0.001) # Prevent negative precipitation
    ...
    grid.at_node['water__unit_flux_in'][:] = precip_field
    ...
    for i in range(num_iterations):
        # 5a. Apply spatially variable tectonic uplift to core nodes
        grid.at_node['bedrock__elevation'][grid.core_nodes] += uplift_field[grid.core_nodes] * dt
        grid.at_node['topographic__elevation'][grid.core_nodes] += uplift_field[grid.core_nodes] * dt
